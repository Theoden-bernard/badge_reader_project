defmodule BadgeReaderWeb.ProfileLive do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     ...
     |> assign(:week, CalculationOfTime.this_week_calculation_by_user(current_user))}
  end
      ...
      <div class="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <div class="flex flex-row mt-10">
          ...
          <div class="flex flex-col">
            <div class="text-6xl">
              {if @current_user,
                do: "#{@current_user.lastname} #{@current_user.firstname}",
                else: "Non connecté"}
            </div>
            <div class="text-2xl">
              {if @current_user, do: @current_user.role.name_role, else: "Aucun rôle"}
            </div>
          </div>
        </div>
        <%!-- Cards --%>
