defmodule BadgeReaderWeb.UserAuth do
  ...
  * **Socket Disconnection:** Broadcasts teardown events to channel clusters to force-disconnect stateful loops when logging out.
  ...
  @remember_me_cookie "_badge_reader_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      BadgeReaderWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      BadgeReaderWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule BadgeReaderWeb.PageLive do
        use BadgeReaderWeb, :live_view

        on_mount {BadgeReaderWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{BadgeReaderWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_admin, _params, session, socket)do
    ...
    user = BadgeReader.Repo.preload(socket.assigns.current_scope.user, [:role])

    if user.role.name_role == "Administrateur" do
      {:cont, socket}
    else
      socket =
        socket
